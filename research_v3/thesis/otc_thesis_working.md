...
비공개 판매 카탈로그 776건을 별도 연구 입력 계층으로 처리하였다. 약학정보원 연결이 confirmed인 SKU는 369건이었고, 안정적인 공식 제품 키 기준 고유 품목은 341개였다. `review_required` 82건, `not_found` 137건과 `not_applicable` 188건은 연구 검색·분류 후보에서 제외하였다. confirmed 제품을 성분 코드와 제형으로 묶으면 264개 그룹이었다. 공식 제품 키 매핑 실패는 0건, 원문 매칭 충돌은 2건이었다.

약학정보원 제품 코드와 기존 식약처 `item_sequence`의 정확 교집합은 0건이었다. 따라서 이름 유사도만으로 기존 제품 마스터와 병합하지 않았다.

confirmed 369건에서 효능·효과, 용법·용량, 유효성분, 제형, 투여경로, ATC, 보관, 보험과 일반 주의사항의 결측은 0건이었다. 표준코드·바코드는 292건(79.1%), KPIC 분류는 73건(19.8%), 상호작용은 306건(82.9%)이 비어 있었다. 사용기간과 전문가 주의사항은 모든 confirmed 행에서 비어 있었다. 임부 DUR은 288건(78.0%), 연령 DUR은 354건(95.9%), 금기 DUR은 339건(91.9%)이 비어 있었다.

약학정보원 confirmed는 식약처 허가 확인이나 안전성 규칙 승인을 뜻하지 않는다. 따라서 이 369건은 연구용 검색·분류·후보 선별에만 사용하고 기존 13개 사이트 제품, 15개 released 규칙과 평가 분모에는 합산하지 않았다. 가격은 임상 검색 순위와 안전 제외 조건에 사용하지 않았다.
...
연구 테스트 136개와 앱 테스트 66개가 통과했고 lint, typecheck와 production build도 통과하였다. 정적 경로는 156개였다. 명령, 종료코드와 통과 수는 software validation JSON에 기록하였다. 이 검증은 데이터 계약, 규칙 실행, 화면 구성과 빌드 재현성을 확인하지만 임상 안전성이나 실제 사용자 이해도를 증명하지 않는다.
...
약학정보원 연결 confirmed 369건은 연구 후보를 넓혔지만 식약처 품목 허가와 released 규칙을 추가한 결과가 아니다. 사용기간, 전문가 주의사항, 상호작용과 DUR 필드의 결측도 컸다. 따라서 이 자료만으로 제품별 안전 판정이나 복용 추천을 확대할 수 없다.
