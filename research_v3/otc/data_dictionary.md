...
교정된 카탈로그 제품명이 기존 분석 제품의 제품명·제형 제거 별칭과 정확히 같은 판매 SKU만 담는다. 현재 6개 SKU가 기존 식약처 제품 5개와 이름 수준에서 교차한다. 이 파일은 선별 결과이므로 모든 행은 `requires_official_match_review`, `promotion_allowed=false`다.
...
문자열 유사도 기준을 충족했지만 동일 제품으로 볼 수 없는 1개 SKU를 분리한다. 제형·제조사·식별자가 확인되지 않은 유사 이름은 교집합 수에 포함하지 않는다.

### `selection/catalog_additional_otc_candidates.csv`

일반의약품이 포함될 수 있는 카테고리, 제품명 끝 제형 단서와 규격 호환 조건을 모두 충족한 screening shortlist다. 현재 111개 SKU·108개 정규화 이름이 있으나 `candidate_requires_official_domain_and_item_match` 상태다. 식약처 제품 영역, `item_sequence`, 허가 상태, 성분, 용법·용량과 DUR를 확인하기 전에는 제품 마스터·런타임·논문 제품 수에 합산하지 않는다.

### `audit/catalog_candidate_bridge.json`

최신 정식 원본 776건, 교정 이름 기준 중복 29그룹·59 SKU, JSON/CSV 동일성, 원본 해시, 가격 미복사, 전체 원본 미복사와 산출물 해시를 검증한다. 약학정보원 상태는 별도 import에서 검증하며 이 선별 결과를 식약처 승격 근거로 사용하지 않는다.
...
`official_additional_data.health_kr_raw.ingredient_details[].ingredient_code`의 정렬된 집합과 정규화 제형으로 그룹 ID를 만든다. 제품명 유사도와 가격은 그룹 키에 사용하지 않는다. 2026-07-16 최신 스냅샷에는 306개 그룹이 있다.

### `selection/catalog_health_kr_conflict_review.csv`

약학정보원 매칭 근거에 충돌이 남은 행의 source ID, 안정적인 공식 제품 키, 제품 코드, 출처 URL과 충돌 이유만 보존한다. 효능·용법·주의사항 원문과 가격은 포함하지 않는다. 현재 충돌 항목 6건은 자동 승격하지 않고 검토 대상으로 남긴다.

### `selection/catalog_health_kr_summary.json`

총 776건의 import 결과, 상태별 건수, 고유 공식 품목 수, 연구 검색 사용 가능 수, 중복 연결, 매핑 실패, 충돌, 재매칭 결과와 confirmed 행의 주요 필드별 결측률을 기록한다. 정식 JSON·CSV, 교정 파일과 portable v1의 SHA-256도 함께 저장한다.

### `local/catalog_health_kr_official_products.json`

458개 confirmed SKU를 `official_item_seq` 기준 413개 공식 품목 엔터티로 묶은 로컬 정규화 JSON이다. portable `official_content`의 문단·표 구조와 출처 URL을 보존하지만 가격, 이미지와 `ai_context`는 포함하지 않는다.

### `selection/catalog_health_kr_existing_product_matches.csv`

기존 연구 제품 16개와 약학정보원 공식 품목을 서로 다른 식별자 체계로 보존한 채 공식 품목명·제조사·제형·포장·성분 수로 대조한다. 결과는 성공 4·충돌 1·미연결 11이며, 런타임 13개로 한정하면 성공 4·충돌 1·미연결 8이다. 이 대조는 식약처 승격 근거가 아니다.

### `audit/catalog_health_kr_integration.json`

정식 JSON·CSV와 portable package의 행·식별자·교정 표시값·상태·해시, confirmed 격리, 안정적인 식별자, 성분·제형 그룹, `app_*` 미사용, 가격 비관여와 CSV 원문 미복사를 검사한다. 임신·연령·고령자 DUR, 금기, 상호작용 제외와 red flag 후보 0건·약사 또는 진료 의뢰도 결정론적으로 확인한다. 이 검증은 임상 성능 평가가 아니므로 `performance_claim_allowed=false`를 유지한다.
